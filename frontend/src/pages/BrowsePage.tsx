...
const extractFirstGenre = (item: any): string => {
    const genreKeys = [
        "drama", "thriller", "children", "suspense", "romance",
        "documentaries", "comedies", "crime", "feel-good"
    ];
...
const BrowsePage = () => {
    ...
    const seriesGenres = ["documentaries", "comedies", "children", "crime", "feel-good"];
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const films = transformed.filter((m: MovieType) => m.genre && itemIsFilm(m));
                const series = transformed.filter((m: MovieType) => m.genre && !itemIsFilm(m));
